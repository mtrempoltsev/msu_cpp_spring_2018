...
class Matrix {
 private:
  ...
  class Proxy {
   ...
   public:
    Proxy(size_t rows_, int *row_data_) : rows{rows_}, row_data{row_data_} {};

    const int operator[](size_t idx) const {
      if (idx >= rows)
        throw std::out_of_range("Index should be between 0 and rows size");
      return row_data[idx];
    }
 ...
 public:
  Matrix(size_t cols_, size_t rows_) : cols{cols_}, rows{rows_} {
    ...
    for (size_t i = 0; i < cols; i++) {
      data[i] = new int[rows];
      for (size_t j = 0; j < rows; j++) data[i][j] = i + j + rand();
    }
  }
  ...
  Proxy operator[](size_t idx) {
    if (idx >= cols)
      throw std::out_of_range("Index should be between 0 and columns size");
